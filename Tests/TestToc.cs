...
public class TestToc
{
    ...
    [Test]
    public void TestGenerateEpubContentListFromPageList()
    {
        PageList pageList = new PageList();
        PageElement page1 = new PageElement(1,"page1");
        PageElement page2 = new PageElement(2,"page2");
        ...
        Epub epub = new Epub();
        var epubContents = epub.GenerateEpubContentListFromPageList(pageList, 2);

        foreach (var unit in epubContents)
        {
            Console.WriteLine($"{unit.Type} \t {unit.FileName}");
        }
    }
}
